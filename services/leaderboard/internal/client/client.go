...
// userAgent identifies us past WAFs (the default Go UA is blocked).
...
// descending-by-amount order. It does not normalize addresses (see FetchEntries).
...
// metric amount) for the given metric and window, in leaderboard order. Wallets
// are lowercased and de-duplicated (first-seen wins). An empty result is treated
// as a failed fetch so a transient garbage-but-200 can't wipe the watchset.
func (c *Client) FetchEntries(ctx context.Context, metric Metric, window Window, topN int) ([]Entry, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("leaderboard %s: %w", path, err)
	}
